namespace Models.RealEstate
{
    public class SessionKey
    {
        public static readonly string r_asmc_set_train_lines = "r_asmc_set_train_lines";
        public static readonly string r_asmc_set_train_stations = "r_asmc_set_train_stations";

        public static readonly string r_asmc_set_area_cities = "r_asmc_set_area_cities";
        public static readonly string r_asmc_set_area_towns = "r_asmc_set_area_towns";

        public static readonly string r_asmc_ms_list_map_cities = "r_asmc_ms_list_map_cities";
        public static readonly string r_asmc_ms_list_map_towns = "r_asmc_ms_list_map_towns";

        public static readonly string r_asmc_set_ms_conditions = "r_asmc_set_ms_conditions";
        public static readonly string r_asmc_set_ms_data = "r_asmc_set_ms_data";
    }
}
