...
$(function () {
    ...
    //display Map
    var model = {
        CityCDList : $('#hdnCityCDList').val(),
        TownCDList : $('#hdnTownCDList').val(),
        Unregistered : 1
    }
...
function displayMap(mansions) {
    ...
    for (var i = 0; i < mansions.length; i++) {
        ...
        const color = item.ExpDateText ? '#FFFF00' : item.ValidFLG === 0 ? '#FF0000' : item.ValidFLG === 1 ? '#0000FF' : '#C0C0C0';
...
function createInfoWindowHtml(item) {
    const css_validflg = item.ValidFLG == 0 ? 'text-danger' : item.ValidFLG == 1 ? 'cap-text-primary' : 'text-dark';
